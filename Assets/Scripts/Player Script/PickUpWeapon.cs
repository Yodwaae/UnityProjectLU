...
public class PickUpWeapon : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)//Se déclenche quand le joueur touche quelque chose
    {
        ...
        pour changer les caractéristiques de l'arme équipée du joueur puis détruit l'arme (l'instance de pickUp)
        ...
        if (collision.gameObject.tag == "BasicW" )
        {
            ...
            audioSource.PlayOneShot(soundGun1);//joue le son de récupération de l'arme
            Destroy(collision.gameObject);//supprime l'objet de l'arme sur le sol
        }
        else if(collision.gameObject.tag == "Assaut")
        {
            ...
            audioSource.PlayOneShot(soundGun2);//joue le son de récupération de l'arme
            Destroy(collision.gameObject);//supprime l'objet de l'arme sur le sol
        }
